...
function createGitCheckpoint(round: number, timestamp: string) {
  ...
  try {
    ...
    if (!fs.existsSync('.git') && !fs.existsSync('../.git')) {
      ...
      execSync('git config user.name "PrivateServer-Dev"', { stdio: 'pipe' });
      execSync('git config user.email "dev@simcompanies.local"', { stdio: 'pipe' });
    }
    execSync('git add -A', { stdio: 'pipe' });
    const commitMsg = `checkpoint: pre-e2e round ${round} [${timestamp}]`;
